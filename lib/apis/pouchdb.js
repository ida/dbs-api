...
// If no id is given, give one.
// todo: If id is given and exists already, error.
  ...
  while( notAddedEntry ) {
    ...
    notAddedEntry = await db.get(entryId).catch(async e=>{

      entryObject._id = entryId

      entry = await db.put(entryObject).then(result=>{return result})

      entry = await db.get(entry.id)
